...
class ControlStocklogController extends Controller
{
    public function index()
    {
        $stockLogs = StockLog::with(['stock'])
        ...
        if(!$stockLogs->isEmpty()){
            ...
            foreach ($stockLogs as $stockLog) {
                $temp = [
                    ...
                    "source"=> $stockLog->source,
                    "cost"=> $stockLog->cost,
                    "file"=> $stockLog->file,
                    "stock"=>$stockLog->stock,
                ];
                if($stockLog->sales_crmsid==null){
                    $temp["SalesCrm"]= [];
                    $temp["Supplyer"]= $stockLog->Supplyer;

                }
                else{
                    $temp["Supplyer"]= [];
                    $temp["SalesCrm"]= $stockLog->SalesCrm;

                }
                $formattedstockLogs[] =  $temp;
            }
            return response()->json(["data"=>$formattedstockLogs,"status"=>Response::HTTP_OK]);
        }
        else{
