...
use Carbon\Carbon;
class OperationMonthlyScPlanController extends Controller
{
    /**
    ...
    {
        $OperationMonthlyScPlan=OperationMonthlyScPlan::latest()->get();
        if(!$OperationMonthlyScPlan->isEmpty()){
            $OperationMonthlyScPlan->transform(function($data){
            ...
            return response()->json(["data"=>$OperationMonthlyScPlan,"status"=>200]);
        }
        else{
    ...
    public function store(Request $request)
    {
        ...
        OperationMonthlyScPlan::create($validator);
    ...
    {
        $User=OperationMonthlyScPlan::find($id);
        if($User!=null){
            return response()->json(["data"=>$User,"status"=>202]);
        }
        else{
            return response()->json(["data"=>"there is no OperationMonthlyScPlan","status"=>404]);
    ...
    public function update(Request $request, $id)
    {
        ...
        // Validation passed, create the user
        OperationMonthlyScPlan::where('id',$id)->update($validator);
